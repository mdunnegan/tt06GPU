...
__global__ void make_randomNums( double* randArray,double* randArray2,double* randArray3, int num_cells, int num_changing_vars, int sf) {
	curandState local;
...
int main(int argc, const char* argv[])
{
	...
	FILE *fV3 = fopen("tt06 GPU 1D Voltage3.txt", "w");
	//FILE *ft3 = fopen("tt06 GPU Time3.txt", "w");
	FILE *params3 = fopen("tt06 GPU SA 1D parameters3.txt", "w");
	FILE *allparams3 = fopen("tt06 GPU 1D all parameters3.txt", "w");
	FILE *output3 = fopen("tt06 GPU SA 1D output3.txt", "w");
	FILE *s2output3 = fopen("tt06 GPU 1D s2 Analysis3.txt", "w");
	FILE *v3 = fopen("tt06 GPU 1D localVelocity3.txt", "w");
	FILE *p3 = fopen("tt06 GPU 1D passedThreshold3.txt", "w");
	...
	if (num_changing_vars != 0) {
		...
		make_randomNums << <simulations, num_changing_vars >> >(dev_randNums, dev_randNums2, dev_randNums3, num_cells, num_changing_vars, seed_factor);
	...
	while (time<iterations) {
		...
		cudaDeviceSynchronize();
		compute_voltage << <simulations, (num_cells / cells_per_thread) >> >(dev_vars2, dev_Vtemp2, dev_ion_currents2, step, dev_randNums2, simulations, length, width, num_changing_vars, time, stimDur, stimAmp, tstim, cells_per_thread, local, dev_passed2, threshold);
		update_voltage << <simulations, (num_cells / cells_per_thread) >> >(dev_vars2, dev_Vtemp2, num_cells, cells_per_thread);
		cudaDeviceSynchronize();
		compute_voltage << <simulations, (num_cells / cells_per_thread) >> >(dev_vars3, dev_Vtemp3, dev_ion_currents3, step, dev_randNums3, simulations, length, width, num_changing_vars, time, stimDur, stimAmp, tstim, cells_per_thread, local, dev_passed3, threshold);
